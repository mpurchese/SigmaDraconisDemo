namespace SigmaDraconis.AnimalAI
{
    ...
    [ProtoContract]
    public class SnowTortoiseAI : IAnimalAI
    {
        ...
        protected SnowTortoiseAI() { }

        public SnowTortoiseAI(IAnimal animal)
        {
            this.Animal = animal;
        ...
        private void MakeChoice()
        {
            ...
            if (!ZoneManager.AnimalZone.ContainsNode(this.Animal.MainTileIndex) && this.Animal.MainTile.AdjacentTiles8.Any(t => ZoneManager.AnimalZone.ContainsNode(t.Index) && t.ThingsAll.All(u => u.TileBlockModel.In(TileBlockModel.None, TileBlockModel.Point) == true)))
            {
                this.LeaveClosedTile();
                return;
            }

            if ((World.Temperature <= -5 || (World.WorldLight.Brightness < 0.5 && World.Temperature <= 5)) && this.TrySatisfySleep()) return;
        ...
        private bool TrySatisfySleep()
        {
            ...
            if (this.Animal.IsResting || this.Animal.MainTile.ThingsAll.Any(t => t.ThingType == ThingType.BigSpineBush))
            {
                this.CurrentActivity = new ActivitySleep(this.Animal, 3600 * (3 + Rand.Next(3)));   // 3 - 5 hours
                return true;
            }

            var candidateTiles = World.GetThings<IAnimatedThing>(ThingType.BigSpineBush).SelectMany(b => b.AllTiles).Where(t => RoomManager.GetRoom(t.Index) == null).ToList();
            var path = this.FindPath(this.Animal.MainTile, candidateTiles, new HashSet<int>());
        ...
        private bool TrySatisfyRoam()
        {
            ...
            {
                var path = PathFinder.FindPath(this.Animal.MainTileIndex, i.Value, ZoneManager.AnimalZone.Nodes, new HashSet<int>());
                if (path?.RemainingNodes?.Any() == true)
                {
