namespace SigmaDraconis.AI
{
    ...
    [ProtoContract]
    public class ActivityDrink : ActivityBase
    {
        [ProtoMember(1)]
        public int TargetId { get; private set; }

        // Deserialisation ctor
        protected ActivityDrink() { }

        public ActivityDrink(IColonist colonist, IColonistInteractive target, Path path = null) : base(colonist, target.Id)
        {
        ...
        private void UpdateCurrentAction(IColonist colonist, Path path = null)
        {
            ...
            if (path != null)
            {
                ...
                if (!(World.GetThing(this.TargetId) is IWaterDispenser dispenser))
                {
            ...
            else
            {
                ...
                if (!(World.GetThing(this.TargetId) is IWaterDispenser dispenser) || dispenser.DispenserStatus == DispenserStatus.NoResource || !dispenser.CanAssignColonist(this.colonistId, this.Colonist.MainTileIndex))
                {
                    this.IsFinished = true;
                    return;
                }
                ...
                {
                    this.CurrentAction = new ActionDrink(colonist, dispenser.Id);
                }
                else
                {
