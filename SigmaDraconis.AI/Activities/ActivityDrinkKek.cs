namespace SigmaDraconis.AI
{
    ...
    [ProtoContract]
    public class ActivityDrinkKek : ActivityBase
    {
        [ProtoMember(1)]
        public int? TableId { get; private set; }

        // Deserialisation ctor
        protected ActivityDrinkKek() { }

        public ActivityDrinkKek(IColonist colonist, int? tableId, Path path = null) : base(colonist, tableId)
        {
        ...
        {
            if (this.Colonist.CarriedItemTypeArms != ItemType.Kek)
            {
                this.IsFinished = true;
                return;
            }

            var table = this.TableId.HasValue ? World.GetThing(this.TableId.Value) as ITable : null;
            ...
            if (path != null)
            {
                ...
                else this.CurrentAction = new ActionWalk(colonist, path, Vector2f.Zero);
            }
            else if (table == null)
            {
                this.CurrentAction = new ActionRelax(colonist, null);
            }
