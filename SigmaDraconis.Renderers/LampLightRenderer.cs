namespace SigmaDraconis.Renderers
{
    ...
    public class LampLightRenderer : IRenderer, IDisposable
    {
        ...
        public void LoadContent(GraphicsDevice graphicsDevice, ContentManager contentManager)
        {
            ...
            var texture = content.Load<Texture2D>("Textures\\Lights\\Lamp");
        ...
        public void UpdateBuffers()
        {
            ...
            var lamps = World.GetThings<ILamp>(ThingType.Lamp).Where(l => l.EnergyUseRate > 0).ToList();
            var size = lamps.Count * 4;
            if (size == 0)
            {
            ...
            {
                var tile = lamp.MainTile;
                var cx = tile.CentrePosition.X;
                var cy = tile.CentrePosition.Y;

                var c = Color.White;
                vertexArray[i] = new VertexPositionColorTexture { Position = new Vector3(cx - 180f, cy, 0), TextureCoordinate = new Vector2(0f, 1f), Color = c };
                vertexArray[i + 1] = new VertexPositionColorTexture { Position = new Vector3(cx, cy - 90f, 0), TextureCoordinate = new Vector2(0f, 0f), Color = c };
                vertexArray[i + 2] = new VertexPositionColorTexture { Position = new Vector3(cx + 180f, cy, 0), TextureCoordinate = new Vector2(1f, 0f), Color = c };
                vertexArray[i + 3] = new VertexPositionColorTexture { Position = new Vector3(cx, cy + 90f, 0), TextureCoordinate = new Vector2(1f, 1f), Color = c };
