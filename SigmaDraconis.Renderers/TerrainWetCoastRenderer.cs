namespace SigmaDraconis.Renderers
{
    ...
    public class TerrainWetCoastRenderer : IRenderer, IDisposable
    {
        ...
        public Effect LoadContent(GraphicsDevice graphicsDevice, ContentManager contentManager)
        {
            ...
            var texture3 = contentManager.Load<Texture2D>("Textures\\Tiles\\TerrainCoastMask");
            ...
            this.effect.Parameters["xTextureMask"].SetValue(texture3);

            this.textureSize = new Vector2(texture1.Width, texture1.Height);
            this.textureSize2 = new Vector2(texture3.Width, texture3.Height);
        ...
        public void UpdateBuffers()
        {
            ...
            var tiles = this.Layer == 2
                ? World.SmallTiles.Where(t => t.BiomeType == BiomeType.Wet && t.TerrainType == TerrainType.Coast).ToList()
                : World.SmallTiles.Where(t => t.BiomeType != BiomeType.Desert && t.TerrainType == TerrainType.Coast).ToList();
            ...
            foreach (var tile in tiles)
            {
                ...
                var bx1 = (bigTileFrame.X + (bigTileFrame.Width * ox)) / this.textureSize2.X;
                var bx0 = bx1 + (bigTileFrame.Width / (6f * this.textureSize2.X));
                var bx2 = bx1 + (bigTileFrame.Width / (3f * this.textureSize2.X));
                var by1 = ((bigTileFrame.Y + (bigTileFrame.Height * oy)) - 1f) / this.textureSize2.Y;
                var by0 = by1 + (bigTileFrame.Height / (6f * this.textureSize2.Y));
                var by2 = by1 + (bigTileFrame.Height / (3f * this.textureSize2.Y));

                vertexArray[i] = new VertexPositionColorTextureTexture(new Vector3(x1, y0, 0), c, new Vector2(tx1 / 5f, ty1 / 5f), new Vector2(bx1, by0));
                vertexArray[i + 1] = new VertexPositionColorTextureTexture(new Vector3(x0, y1, 0), c, new Vector2(tx2 / 5f, ty1 / 5f), new Vector2(bx0, by1));
                vertexArray[i + 2] = new VertexPositionColorTextureTexture(new Vector3(x0, y2, 0), c, new Vector2(tx1 / 5f, ty2 / 5f), new Vector2(bx0, by2));
                vertexArray[i + 3] = new VertexPositionColorTextureTexture(new Vector3(x2, y0, 0), c, new Vector2(tx2 / 5f, ty2 / 5f), new Vector2(bx2, by0));
