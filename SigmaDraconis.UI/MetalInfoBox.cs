namespace SigmaDraconis.UI
{
    ...
    public class MetalInfoBox : TexturedElement
    {
        ...
        public MetalInfoBox(GraphicsDevice graphicsDevice, UIElement parent, int x, int y, int width, int height)
            : base(graphicsDevice, parent, x, y, width, height)
        {
            this.titleLabel = new SimpleTextLabel(this, 0, 4, this.Width, 18, "METAL PRODUCTION", new Color(80, 240, 240));
        ...
        {
            //var row = 0;
            //var total = ResourceManager.Instance.MetalProductionHourlyAmounts.Sum(x => x.Value);
            //var totalStr = $"{total:N0} kg/h";
            //foreach (var pair in ResourceManager.Instance.MetalProductionHourlyAmounts.OrderByDescending(i => i.Value).Where(i => (int)i.Value != 0))
            //{
            //    var definition = ThingTypeRegister.Definitions[pair.Key];
            //    var amountStr = pair.Value < 100 ? $"{pair.Value:N1} kg/h" : $"{pair.Value:N0} kg/h";
            //    var count = ResourceManager.Instance.MetalProductionHourlyCounts[pair.Key];
