namespace SigmaDraconis.UI
{
    ...
    public class EnergyInfoBox : TexturedElement
    {
        ...
        public EnergyInfoBox(GraphicsDevice graphicsDevice, UIElement parent, int x, int y, int width, int height)
            : base(graphicsDevice, parent, x, y, width, height)
        {
            this.titleLabel = new SimpleTextLabel(this, 0, 4, this.Width, 18, "ENERGY PRODUCTION", Color.Gold);
        ...
        {
            //var row = 0;
            //var total = ResourceManager.Instance.EnergyProductionHourlyAmounts.Sum(x => x.Value);
            //var totalStr = total < 100 ? $"{total:N1} kW" : $"{total:N0} kW";
            //foreach (var pair in ResourceManager.Instance.EnergyProductionHourlyAmounts.OrderByDescending(i => i.Value).Where(i => (int)(i.Value * 10) != 0))
            //{
            //    var definition = ThingTypeRegister.Definitions[pair.Key];
            //    var amount = pair.Value;
            //    var amountStr = pair.Value < 100 ? $"{pair.Value:N1} kW" : $"{pair.Value:N0} kW";
            //    var count = ResourceManager.Instance.EnergyProductionHourlyCounts[pair.Key];

            //    this.column1Labels[row].Text = definition.Name + (count > 1 ? $" x{count}" : "");
            //    this.column2Labels[row].Text = amount > 0 ? $"+{amountStr}" : amountStr;
            //    this.column2Labels[row].Colour = amount > 0 ? Color.Green : Color.Red;
