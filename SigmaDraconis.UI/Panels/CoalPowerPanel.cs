namespace SigmaDraconis.UI
{
    ...
    public class CoalPowerPanel : GeneratorPanelBase, IThingPanel
    {
        private readonly LeftRightPicker burnRatePicker;
        private ThingType currentThingType = ThingType.None;

        public CoalPowerPanel(IUIElement parent, int y) : base(parent, y)
        {
            ...
            this.statusControl = new BuildingStatusControl(this, Scale(36), Scale(44), Scale(248), Scale(20), false, true);
            this.AddChild(this.statusControl);
            this.statusControl.AutoRestartChanged += OnAutoRestartButtonClick;

            this.maintenanceControl = new BuildingMaintenanceControl(this, Scale(36), Scale(68));
            this.AddChild(this.maintenanceControl);
            this.maintenanceControl.PriorityChanged += this.OnRepairPriorityChanged;

            var energy = Constants.CoalPowerEnergyOutput * Constants.CoalPowerFramesToProcess / Constants.FramesPerHour;
            var water = energy * Constants.CoalPowerWaterUsePerKwH / 100.0;
            this.flowDiagram = new FlowDiagramPowerStation(this, 0, Scale(106), this.W, ItemType.Coal, Constants.CoalPowerFramesToProcess, water, energy);
        ...
        public override void Update()
        {
            if (this.building is IPowerPlant powerPlant && this.IsBuildingUiVisible)
            {
                ...
                if (this.flowDiagram is FlowDiagramPowerStation f) f.Frames = Constants.CoalPowerFramesToProcess * 3 / powerPlant.BurnRateSetting;
